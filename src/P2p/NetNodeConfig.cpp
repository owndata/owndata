...
void NetNodeConfig::initOptionsForHelp(boost::program_options::options_description& desc) {
  command_line::add_arg(desc, arg_p2p_bind_ip);
  command_line::add_arg(desc, arg_p2p_bind_port);
  command_line::add_arg(desc, arg_p2p_external_port);
  command_line::add_arg(desc, arg_p2p_allow_local_ip);
...
//  if (vm.count(arg_network_id.name) != 0 && (!vm[arg_network_id.name].defaulted())) {
    networkId = boost::lexical_cast<boost::uuids::uuid>("ae582997-3a36-db6a-80dd-d2fd394c1e4c");
//  }
